...
class MainFrame(tk.Frame):
	def __init__(self, parent, controller):
		...
		self.capture_result_frame = None  # 保持對CaptureResult框架類的引用
		self.configure(width=800, height=500)  # Set the frame size

#		label = tk.Label(self, text="Main Frame")
#		label.pack(pady=10)

#		self.textbox = tk.Entry(self)
#		self.textbox.pack(pady=10)

		# 創建CaptureResult框架的實例
		self.capture_result_frame = CaptureResult(self, controller)
...
#		button1.place(x=100, y=100)  # 設置按鈕的位置 (x, y)
	...
	def show_capture_result(self):
		# 在按鈕中調用CaptureResult框架的方法
		if self.capture_result_frame is not None:
			self.capture_result_frame.load_file_to_listbox()
...
class CaptureResult(tk.Frame):
	...
	def show_image(self, folder_path):
		image2_path = 'examinee/' + folder_path + '/picture.png'
		try:
			# 打開圖片文件
			image = Image.open("model/picture.png")
	...
	def __init__(self, parent, controller):
		...
		label = tk.Label(self, text="Capture Result")
		label.pack(pady=10)
		
		label2 = tk.Label(self, text="標準答案")
		label2.place(x=270, y=100)  # 設置按鈕的位置 (x, y)
...
#		self.show_image()
		...
		# 創建一個按鈕來觸發顯示圖片的操作
#		show_image_button = tk.Button(self, text="Show Image", command=self.show_image())
