...
def draw_table():
	# 讀取座標數據
	data = pd.read_csv("2-3model.txt", delimiter='@', header=None, names=['序號', 'x座標', 'y座標', '文字'])  # 使用@符號作為分隔符

	# 讀取要圈出的序號
	correct_examinee_path = file_path + '/' + 'correct_examinee.txt'
	with open(correct_examinee_path, "r") as f:
...
#			ax.add_patch(plt.Circle((x + 0.5, y + 0.5), 0.4, color='red', fill=False, linestyle='solid', lw=2))
	...
	# 自動儲存圖片
	picture_path = file_path + '/' + 'picture_model.png'
