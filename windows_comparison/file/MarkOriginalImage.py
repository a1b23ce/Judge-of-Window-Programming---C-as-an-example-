...
def DrawSquare():
	...
	# 讀取座標數據
	if model_or_examinee == 'M':
		data = pd.read_csv('model' + '/' + 'sorted_original_data.txt', delimiter='@', header=None, names=['序號', 'x1座標', 'y1座標', 'x2座標', 'y2座標', '文字'])  # 使用@符號作為分隔符
	else:
		data = pd.read_csv(file_path + '/' + 'sorted_original_data.txt', delimiter='@', header=None, names=['序號', 'x1座標', 'y1座標', 'x2座標', 'y2座標', '文字'])  # 使用@符號作為分隔符
