...
def index_trans(ELEMENT):
	...
	def custom_sort(elem):
		return (elem.word, elem.x, elem.y)

	# 分組 ELEMENT_2D 中的元素，根據 word 相同的分組
	element_groups = {}

	for elem in ELEMENT_2D:
		word = elem.word
		if word not in element_groups:
			element_groups[word] = []
		element_groups[word].append(elem)

	# 處理文字相同元件的 index 最小值
	for elements in element_groups.values():
		set_min_index_for_same_text_elements(elements)
	
	# 創建一個二維陣列來存儲排序後的元素
	sorted_elements_2d = []

	for word, elements in element_groups.items():
		sorted_elements = sorted(elements, key=custom_sort)
		sorted_elements_2d.append(sorted_elements)
	
	# 開啟文件並將排序後的元素輸出
	with open(outputfile_path, "w", encoding='utf-8') as output_file:
		for group in sorted_elements_2d:
			for elem in group:
				output_file.write(f"{elem.index}@{elem.x}@{elem.y}@{elem.word}\n")
