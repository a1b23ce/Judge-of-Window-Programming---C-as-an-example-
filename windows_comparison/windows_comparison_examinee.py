	...
	for root, dirs, files in os.walk(folder_path):
		if root not in processed_folders:
			try:					
				...
				TDIT = subprocess.run(["python", "file/TwoDIndexTransformation_for_examinee.py", Window_Name, root], stdout=subprocess.PIPE, text=True)					
				# 檢查子進程是否成功執行
				if TDIT.returncode == 0:
					# 子進程成功執行，你可以獲取輸出
					if TDIT.stdout == "MatchedElementGreaterThanHalf":
						...
						examinee_accuracy = os.path.basename(root)	+ " " + "<50%\n"
						with open("examinee_accuracy.txt", "a", encoding='utf-8') as file:
							file.write(examinee_accuracy)
#						print("<50%")
